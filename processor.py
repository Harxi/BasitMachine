...
def replaceRegister(interrupts, name: str):
	return {"type": "int", "value": eval(f'interrupts.registers.{name}')}
...
class Bas1xInstructions:
	...
	def move(interrupts, value):
		if len(value) < 2 or len(value) > 2:
			...
			if value[1]["type"] == "reg":
				if value[1]["value"] in interrupts.registers.regs: value[1] = replaceRegister(interrupts, value[1]["value"])["value"]
				else:
					print(f"RegisterError: Register '{value[1]['value']}' not found")
					exit(-1)
			elif value[1]["type"] == "adrs":
				if value[1]["value"]["type"] == "reg":
					if value[1]["value"]["value"] in interrupts.registers.regs: value[1]["value"] = replaceRegister(interrupts, value[1]["value"]["value"])["value"]
					else:
			...
			if value[0]["type"] == "reg":
				...
				if value[0]["value"]["type"] == "reg":
					if value[0]["value"]["value"] in interrupts.registers.regs: value[0]["value"] = replaceRegister(interrupts, value[0]["value"]["value"])["value"]
					else:
	...
	def push(interrupts, value):
		for ind in range(len(value)):
			if value[ind]["type"] == "reg":
				if value[ind]["value"] in interrupts.registers.regs: value[ind] = replaceRegister(interrupts, value[ind]["value"])["value"]
				else:
					print(f"RegisterError: Register '{value[ind]['value']}' not found")
					exit(-1)
		for number in value:
	...
	def interrupt(interrupts, value):
		if len(value) > 1 or len(value) < 1:
			...
			if value[0]["type"] == "reg":
				if value[0]["value"] in interrupts.registers.regs: value[0] = replaceRegister(interrupts, value[0]["value"])["value"]["value"]
				else:
					print(f"RegisterError: Register '{value[0]['value']}' not found")
					exit(-1)
